...
public class MaterialControllerTest {
	...
	/*
	 * 
	 * Uncomment the below test for testing the
	 * '/material/controller/getMaterialCategories' mapping and it should fail After
	 * implementing the handler method in MaterialController execute the test again
	 * The below test case should pass once the MaterialController handler method is
	...
	public void getMaterialCategoriesTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/material/controller/getMaterialCategories"))
				...
				.andExpect(jsonPath("$[0].categoryId").exists()).andExpect(jsonPath("$[0].categoryName").exists())

				// check the type of json node
				.andExpect(jsonPath("$[0].categoryId").isString()).andExpect(jsonPath("$[0].categoryName").isString())
	 ...
	 * 
	 * Uncomment the below test for testing the
	 * '/material/controller/getMaterialCategoryById/{categoryId}' mapping and it
	 * should fail After implementing the handler method in MaterialController
	 * execute the test again The below test case should pass once the
	 * MaterialController handler method is implemented properly
	 * 
	 */
	...
	public void getMaterialCategoryByIdTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/material/controller/getMaterialCategoryById/" + categoryId))
				...
				// check if the json node exsits
				.andExpect(jsonPath("$.categoryId").exists()).andExpect(jsonPath("$.categoryName").exists())

				// check the type of json node
				.andExpect(jsonPath("$.categoryId").isString()).andExpect(jsonPath("$.categoryName").isString())
