...
public class VendorControllerTest {
	...
	/*
	 * 
	 * Once you have completed the other test cases execute the below test case it
	 * should fail After modifying the VendorController class execute the below test
	 * case it should pass
	 * 
	 */
	...
	public void vendorListVendorControllerTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/vendor/controller/getVendors")).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$[0].vendorId").exists()).andExpect(jsonPath("$[0].vendorName").exists())
				.andExpect(jsonPath("$[0].vendorAddress").exists()).andExpect(jsonPath("$[0].contactPerson").exists())
				.andExpect(jsonPath("$[0].contactNumber").exists())

				// check the type of json node
				.andExpect(jsonPath("$[0].vendorId").isString()).andExpect(jsonPath("$[0].vendorName").isString())
